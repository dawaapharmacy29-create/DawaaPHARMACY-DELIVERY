...
export default function Reconciliation() {
  ...
  const canNext = () => {
    if (step === 1) return selectedSupplierId !== '';
  ...
  const handleApprove = async () => {
    await logAudit({
      ...
      role: user?.role,
      department: 'المطابقة',
      operation: 'اعتماد مطابقة كشف مورد',
      details: `supplier: ${selectedSupplier?.name} from: ${fromDate} to: ${toDate}`,
    });
    toast.success('تم اعتماد المطابقة');
    setStep(5);
  };
  ...
  return (
    <AppLayout title="مطابقة كشف المورد">
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 mb-6">
          ...
          <div>
            ...
            <div className="space-y-4 max-w-lg mr-auto">
              <div>
                ...
                <select value={selectedSupplierId} onChange={e => setSelectedSupplierId(e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm text-right focus:outline-none focus:ring-2 focus:ring-emerald-300">
        ...
        {step === 2 && (
          <div>
            ...
            <div onClick={() => { setFileUploaded(true); toast.success('تم رفع الملف بنجاح'); }}
        ...
        {step === 3 && (
          <div>
            ...
            <div className="space-y-4 max-w-lg mr-auto">
              {[{ label: 'عمود رقم الفاتورة' }, { label: 'عمود القيمة' }, { label: 'عمود التاريخ' }].map(field => (
                <div key={field.label}>
                  ...
                  <select onChange={() => setColumnsLinked(true)} className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm text-right focus:outline-none focus:ring-2 focus:ring-emerald-300">
                    ...
                    {['العمود A', 'العمود B', 'العمود C', 'العمود D'].map(o => <option key={o}>{o}</option>)}
                  </select>
                </div>
              ))}
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-right">
                <thead className="bg-gray-50">
                  <tr>{['رقم الفاتورة', 'قيمتنا', 'قيمة المورد', 'الفرق', 'الحالة'].map(h => (
        ...
        {step === 5 && (
          <div className="text-center py-8">
            ...
            <p className="text-gray-500 mb-4">
              تمت مطابقة كشف حساب <strong>{selectedSupplier?.name}</strong> للفترة {fromDate} — {toDate}
            </p>
            <button onClick={() => { setStep(1); setSelectedSupplierId(''); setFileUploaded(false); setColumnsLinked(false); }} className="bg-emerald-500 text-white px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-emerald-600">
              بدء مطابقة جديدة
            </button>
          </div>
        )}
      ...
      {step < 5 && (
        <div className="flex items-center justify-between">
          <button onClick={() => setStep(s => Math.max(1, s - 1))} disabled={step === 1} className="flex items-center gap-1.5 border border-gray-200 text-gray-600 px-4 py-2.5 rounded-lg text-sm hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed">
          ...
          <button
            onClick={() => {
              ...
              if (step === 4) { handleApprove(); return; }
