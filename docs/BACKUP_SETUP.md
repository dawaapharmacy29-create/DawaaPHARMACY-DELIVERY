...
The result is compressed into one `.tar.gz` file. It is retained as a private GitHub Actions artifact for 30 days and uploaded to the Google Drive folder `02 - Delivery App`.
...
Use the database connection string for project `qlugjplnnkjzxcbhwopg` (`dawaa-delivery-os`). Prefer the Supabase session pooler connection string if GitHub's runner cannot reach the direct IPv6 database endpoint.
...
The workflow runs daily at `01:45 UTC`, which is `04:45` in Cairo during UTC+3. It can also be started manually from the Actions tab.
...
Before restoring into production, create a new empty Supabase project and test the restore there. Verify delivery logins, rider accounts, device binding, attendance, order creation, receipt uploads, invoice reconciliation, RLS policies and RPC functions before changing Vercel environment variables.

## Important limitations

The logical SQL dump does not contain Storage object bytes. Receipt images, trip proofs and other uploaded files require a separate Storage copy workflow.
