...
import { deadStockItems } from '@/data/mockData';
...
export default function DeadStock() {
  ...
  const filtered = deadStockItems.filter(item => {
    const matchBranch = filterBranch === 'الكل' || item.branch === filterBranch;
  ...
  const totalValue = deadStockItems.reduce((s, i) => s + i.value, 0);
  const raked = deadStockItems.filter(i => i.status === 'راكد').length;
  const nearExpiry = deadStockItems.filter(i => i.status === 'قريب الانتهاء').length;
  const expired = deadStockItems.filter(i => i.status === 'منتهي الصلاحية').length;
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.map(item => (
                <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-3 py-3 text-gray-600">{item.branch}</td>
                  <td className="px-3 py-3 text-gray-800 font-semibold">{item.quantity}</td>
                  <td className="px-3 py-3 text-gray-800">{fmt(item.value)}</td>
                  <td className="px-3 py-3 text-gray-600">{item.expiryDate}</td>
                  <td className="px-3 py-3">
                    <span className={`font-bold ${item.daysSinceSale > 90 ? 'text-red-600' : item.daysSinceSale > 60 ? 'text-amber-600' : 'text-gray-700'}`}>
                      {item.daysSinceSale} يوم
                    </span>
                  </td>
                  <td className="px-3 py-3 text-gray-600">{item.supplier}</td>
                  <td className="px-3 py-3"><StatusBadge status={item.status} /></td>
                  <td className="px-3 py-3 text-xs text-gray-500 max-w-32">{item.suggestedAction}</td>
                  <td className="px-3 py-3">
