...
export default function RiderDashboardV2() {
  ...
  async function handleDelivered(order: DeliveryOrder) {
    try {
      ...
      const { data, error } = await supabase.rpc('rider_mark_order_delivered', {
        p_token: token,
  ...
  async function handleFailed() {
    ...
    try {
      ...
      const { data, error } = await supabase.rpc('rider_mark_order_failed', {
        p_token: token,
        p_order_id: String(failOrder.id),
        p_reason: reason,
      })
      const result = getRpcResult<any>(data)
  ...
  return (
    <>
      <RiderOperatingDashboard
        ...
        onOpenOrders={() => document.getElementById('rider-v2-open-orders')?.scrollIntoView({ behavior: 'smooth', block: 'start' })}
        onNewTrip={() => {
      ...
      >
        <section id="rider-v2-open-orders" className="rounded-[30px] border border-slate-100 bg-white p-4 shadow-sm" dir="rtl">
        ...
        <section className="rounded-[30px] border border-slate-100 bg-white p-4 shadow-sm" dir="rtl">
          <h2 className="text-lg font-black text-[#061827]">تشغيل النسخة الجديدة</h2>
          <div className="mt-3 space-y-2 text-sm font-bold text-slate-600">
            ...
            <p>✅ تم إضافة تأكيد التسليم وفشل التسليم للأوردرات المفتوحة.</p>
            <p>⏳ المشاوير والريسيت و×1.5 والتعديل الكامل سيتم نقلهم في المرحلة التالية.</p>
          </div>
        </section>
      </RiderOperatingDashboard>
