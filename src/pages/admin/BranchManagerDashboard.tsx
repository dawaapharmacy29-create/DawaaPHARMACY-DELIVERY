...
import { canonicalBranchName } from '../../lib/branchUtils'
...
export default function BranchManagerDashboard() {
  ...
  async function load() {
    ...
    try {
      ...
      const fallbackProfile = localSession.account_id
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`branch-manager-live-${branchId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'delivery_orders', filter: `branch_id=eq.${branchId}` }, () => { void load() })
  ...
  async function handleDispatchOrder(row: any) {
    ...
    try {
      const { data, error } = await supabase.rpc('mark_order_dispatched_v19', {
        p_order_id: row.id,
        p_actor_name: actor,
        p_notes: 'تأكيد خروج الأوردر من لوحة مدير الفرع'
      })
      if (error) {
        const now = new Date().toISOString()
        const fallback = await supabase.from('delivery_orders').update({
          dispatched_at: row.dispatched_at || now,
          dispatch_status: 'dispatched',
          dispatch_by_name: actor,
          dispatch_notes: 'تأكيد خروج الأوردر من لوحة مدير الفرع',
          updated_at: now
        }).eq('id', row.id).select('*').single()
        if (fallback.error) throw fallback.error
        setOrders(prev => prev.map(o => o.id === row.id ? fallback.data as DeliveryOrder : o))
      } else {
        const next = Array.isArray(data) ? data[0] : data
        if (next) setOrders(prev => prev.map(o => o.id === row.id ? next as DeliveryOrder : o))
      }
      toast.success('تم تسجيل خروج الأوردر من الفرع')
    } catch (e: any) {
      toast.error(e?.message || 'تعذر تسجيل خروج الأوردر')
    }
  }
  ...
  return <div dir="rtl" className="min-h-screen bg-[#F3F7F8] text-[#061827]">
    ...
    <main className="mx-auto max-w-7xl space-y-5 p-4">
      ...
      <RiderDeviceStatusTable rows={deviceRows} loading={loading} onRefresh={() => void load()} title="شحن بطاريات دليفري الفرع" onNotify={sendBatteryNotification} />
