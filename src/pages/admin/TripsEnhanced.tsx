...
export default function TripsEnhanced() {
  ...
  return <div className="min-h-screen bg-[#F3F7F8]" dir="rtl">
    <header className="bg-gradient-to-l from-[#061827] to-[#008E92] p-4 text-white"><div className="mx-auto flex max-w-7xl items-center gap-4"><button onClick={()=>navigate('/admin')} className="rounded-xl bg-white/10 p-2"><ArrowLeft size={22}/></button><div><h1 className="text-xl font-black">إدارة ورقابة المشاوير</h1><p className="text-xs text-teal-100">{counts.withPhoto} بصورة · {counts.withoutPhoto} بدون صورة · الصور والبطاقات قابلة للفتح</p></div></div></header>
    <main className="mx-auto max-w-7xl space-y-4 p-4">
      ...
      <section className="sticky top-2 z-20 rounded-3xl border bg-white p-4 shadow-sm">
        ...
        <div className="mb-3 flex flex-wrap gap-2"><button onClick={()=>setProofFilter('all')} className={`rounded-full px-4 py-2 text-sm font-bold ${proofFilter==='all'?'bg-slate-900 text-white':'bg-slate-50 text-slate-600'}`}>كل الإثباتات ({counts.all})</button><button onClick={()=>setProofFilter('with_photo')} className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-bold ${proofFilter==='with_photo'?'bg-emerald-600 text-white':'bg-emerald-50 text-emerald-700'}`}><Camera size={15}/> بصورة ({counts.withPhoto})</button><button onClick={()=>setProofFilter('without_photo')} className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-bold ${proofFilter==='without_photo'?'bg-rose-600 text-white':'bg-rose-50 text-rose-700'}`}><ImageOff size={15}/> بدون صورة ({counts.withoutPhoto})</button></div>
