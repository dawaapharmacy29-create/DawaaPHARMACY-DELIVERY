...
type Filter = 'all' | 'pending' | 'approved' | 'rejected' | 'without_invoice' | 'missing_proof'
...
export default function TripsOps() {
  ...
  const visible = trips.filter(t => {
    ...
    const f = filter === 'all' || (filter === 'pending' && isPending(t)) || (filter === 'approved' && isApproved(t)) || (filter === 'rejected' && isRejected(t)) || (filter === 'without_invoice' && !hasInvoice(t)) || (filter === 'missing_proof' && !hasInvoice(t) && !hasProof(t))
  ...
  const counts = { all: trips.length, pending: trips.filter(isPending).length, approved: trips.filter(isApproved).length, rejected: trips.filter(isRejected).length, without_invoice: trips.filter(t => !hasInvoice(t)).length, missing_proof: trips.filter(t => !hasInvoice(t) && !hasProof(t)).length }
  ...
  return <div className="min-h-screen bg-[#F3F7F8] p-4 text-right" dir="rtl"><div className="mx-auto max-w-7xl space-y-4">
    ...
    <section className="flex flex-wrap gap-2">{chip('all','الكل')}{chip('pending','مستني')}{chip('approved','معتمد')}{chip('rejected','مرفوض')}{chip('without_invoice','بدون فاتورة')}{chip('missing_proof','بدون إثبات')}</section>
