      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <div className="rounded-[1.5rem] border border-rose-100 bg-rose-50/40 p-4">
          <div className="mb-3 flex items-center justify-between gap-3">
            ...
            <button onClick={() => navigate('/admin/trips')} className="inline-flex items-center gap-1 rounded-2xl bg-white px-3 py-2 text-xs font-black text-rose-700 shadow-sm">كل المشاوير <ChevronLeft size={14} /></button>
          </div>
          <div className="grid gap-3 sm:grid-cols-3">
            <Metric title="مشاوير الدورة" value={trips.length} />
            <Metric title="بصورة كاميرا" value={tripStats.withProof.length} icon={<Camera size={15} />} tone="green" />
            <Metric title="بدون صورة" value={tripStats.withoutProof.length} tone="red" />
            <Metric title="استثناءات" value={tripStats.exceptions.length} tone="amber" />
            <Metric title="مستني اعتماد" value={tripStats.pending.length} tone="blue" />
            <Metric title="قديم بلا إثبات" value={tripStats.oldRows.length} tone="slate" />
          </div>
        ...
        <div className="rounded-[1.5rem] border border-teal-100 bg-teal-50/40 p-4">
          <div className="mb-3 flex items-center justify-between gap-3">
            ...
            <button onClick={() => navigate('/admin/customer-analytics')} className="inline-flex items-center gap-1 rounded-2xl bg-white px-3 py-2 text-xs font-black text-[#008E92] shadow-sm">كل العملاء <ChevronLeft size={14} /></button>
          </div>
          <div className="grid gap-3 sm:grid-cols-3">
            <Metric title="عملاء نشطين" value={toNumber(customerSummary?.active_customers)} />
            <Metric title="+3 طلبات" value={toNumber(customerSummary?.customers_3_plus)} tone="blue" />
            <Metric title="+5 طلبات" value={toNumber(customerSummary?.customers_5_plus)} tone="green" />
            <Metric title="+10 طلبات" value={toNumber(customerSummary?.customers_10_plus)} tone="amber" />
            <Metric title="فواتير العملاء" value={toNumber(customerSummary?.total_invoices)} tone="slate" />
            <Metric title="أوردرات الدورة" value={ordersCount} tone="slate" />
          </div>
          <div className="mt-3 rounded-2xl bg-white p-3">
            <p className="mb-2 text-xs font-black text-slate-500">أعلى العملاء طلبًا</p>
            {topCustomers.length === 0 ? <p className="text-center text-xs font-bold text-slate-400">لا توجد بيانات عملاء بعد</p> : <div className="space-y-2">{topCustomers.slice(0, 4).map((customer, idx) => <div key={`${customer.customer_code || idx}`} className="flex items-center justify-between gap-3 rounded-xl bg-slate-50 px-3 py-2"><div className="min-w-0"><p className="truncate text-sm font-black text-slate-700">{customer.customer_name || 'عميل غير محدد'}</p><p className="text-[11px] font-bold text-slate-400">{customer.customer_code || '—'} · آخر طلب {customer.last_order_date || '—'}</p></div><div className="text-left"><b className="block text-sm text-[#008E92]">{toNumber(customer.invoices_count)} طلب</b><span className="text-[11px] font-bold text-slate-400">{money(customer.total_sales)}</span></div></div>)}</div>}
          </div>
        </div>
      </div>
    </section>
...
function Metric({ title, value, tone = 'teal', icon }: { title: string; value: number; tone?: 'teal' | 'green' | 'red' | 'amber' | 'blue' | 'slate'; icon?: React.ReactNode }) {
  ...
  return <div className={`rounded-2xl p-3 text-center shadow-sm ${cls}`}><p className="flex items-center justify-center gap-1 text-[11px] font-black opacity-80">{icon}{title}</p><b className="mt-1 block text-2xl">{value.toLocaleString('ar-EG')}</b></div>
}
