...
type MismatchRow = {
  ...
  app_customer_code: string | null
  system_customer_code: string | null
...
export default function CustomerNameMismatchPanel() {
  ...
  async function load() {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select('id,invoice_number,rider_name,app_customer_code,system_customer_code,app_customer_name,system_customer_name,app_branch_name,system_branch_name,app_amount,system_amount,app_order_id')
  ...
  const filtered = rows.filter(row => {
    ...
    return [row.invoice_number, row.rider_name, row.app_customer_code, row.system_customer_code, row.app_customer_name, row.system_customer_name, row.app_branch_name, row.system_branch_name]
  ...
  function showInMainList() {
    ...
    next.set('to', to)
    setParams(next)
    window.setTimeout(() => document.getElementById('reconciliation-orders-list')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 50)
  }
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 p-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm font-bold text-amber-800">تُحتسب للمندوب لأن رقم الفاتورة صحيح، وتُراجع هنا لتصحيح اسم أو كود العميل عند الحاجة.</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setOpen(value => !value)} className="rounded-2xl bg-amber-900 px-4 py-2 text-sm font-black text-white">{open ? 'إخفاء التفاصيل' : 'مراجعة الاختلافات'}</button>
        </div>
      </div>
        ...
        <div className="border-t border-amber-200 bg-white p-4">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
            <label className="flex min-w-[260px] flex-1 items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2">
              ...
              <input value={search} onChange={event => setSearch(event.target.value)} placeholder="ابحث بالفاتورة أو الكود أو العميل أو المندوب" className="w-full bg-transparent text-sm font-bold outline-none" />
            </label>
          ...
          {filtered.length > 0 && (
            <div className="max-h-80 overflow-auto rounded-2xl border border-slate-200">
              <table className="w-full min-w-[1050px] text-sm">
                <thead className="sticky top-0 bg-slate-900 text-white"><tr><th className="p-3">الفاتورة</th><th className="p-3">المندوب</th><th className="p-3">كود التطبيق</th><th className="p-3">كود السيستم</th><th className="p-3">اسم التطبيق</th><th className="p-3">اسم السيستم</th><th className="p-3">الفرع</th><th className="p-3">القيمة</th></tr></thead>
                <tbody>
                  {filtered.map(row => <tr key={row.id} className="border-t border-slate-100 text-center">
                    ...
                    <td className="p-3 font-bold text-rose-700">{row.app_customer_code || '—'}</td>
                    <td className="p-3 font-bold text-emerald-700">{row.system_customer_code || '—'}</td>
