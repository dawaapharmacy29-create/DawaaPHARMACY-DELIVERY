-- 0076_allow_reviewable_duplicate_orders.sql
-- Allow a duplicate invoice to be registered for administrative review when
-- the rider provides a duplicate reason and clear details. Doctor name is
-- optional and remains saved when supplied.
...
begin
  ...
  if v_duplicate_id is not null then
    if p_is_duplicate_invoice is not true
      ...
      or length(coalesce(trim(p_duplicate_note),'')) < 5 then
      return jsonb_build_object(
        'success', false,
        'error', 'DUPLICATE_DETAILS_REQUIRED',
        'message', 'رقم الفاتورة مكرر. اختار سبب التكرار واكتب تفاصيل واضحة لا تقل عن 5 حروف.'
      );
    end if;
    v_needs_review := true;
  ...
  insert into public.delivery_orders(
    ...
    coalesce(v_session.rider_id, v_account.rider_id),
    coalesce(v_rider.name, v_account.display_name),
    p_notes, 'rider_app', 'secure_rpc_duplicate_invoice_v2',
    v_duplicate_id is not null,
    case when v_duplicate_id is not null then nullif(trim(p_duplicate_reason),'') else null end,
    case when v_duplicate_id is not null then nullif(trim(p_duplicate_note),'') else null end,
    case when v_duplicate_id is not null then nullif(trim(p_preparing_doctor_name),'') else null end,
    ...
    jsonb_build_object(
      ...
      'duplicate_requires_admin_review', v_duplicate_id is not null
    ),
    now()
  ...
  return jsonb_build_object(
    ...
    'message', case
      when v_duplicate_id is not null then 'تم تسجيل الأوردر المكرر وإرساله للمراجعة الإدارية.'
...
as $$
begin
  if coalesce(trim(new.invoice_number), '') <> '' and exists(
    select 1 from public.delivery_orders
    where (coalesce(invoice_number, '') = trim(new.invoice_number)
       or coalesce(invoice_no, '') = trim(new.invoice_number))
      and id is distinct from new.id
      and deleted_at is null
  ) then
    if new.is_duplicate_invoice is not true
      or coalesce(trim(new.duplicate_reason), '') = ''
      or length(coalesce(trim(new.duplicate_note), '')) < 5 then
      raise exception 'لا يمكن تسجيل فاتورة مكررة بدون سبب وتفاصيل واضحة لا تقل عن 5 حروف';
    end if;
    new.needs_review := true;
    new.review_reason := 'duplicate_invoice';
    new.duplicate_review_status := coalesce(nullif(new.duplicate_review_status, ''), 'pending');
  end if;
  return new;
end;
$$;
