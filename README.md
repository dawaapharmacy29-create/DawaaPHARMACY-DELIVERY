...
## تحديث الاستقرار وتشغيل Flow الدليفري

تمت إضافة ملف SQL مهم:

```text
supabase/new-project/20_stability_product_upgrade.sql
```

شغّله داخل Supabase الجديد بعد إنشاء مستخدم Auth:
...
- إصلاح تحميل ملف المستخدم بدون nested select مع `delivery_branches` لتجنب أخطاء schema cache.
- تقليل سقوط التطبيق عند فشل Query واحدة.
- جعل Dashboard تعرض fallback بدل إسقاط الصفحة بالكامل.
- جعل صفحة الأوردرات تعرض ErrorState/EmptyState بدل استمرار `جاري التحميل`.
- تحسين شاشة المندوب لتدعم تسلسل التشغيل:
  - تسجيل حضور.
  - بدء خروجة.
  - بحث عميل.
  - إضافة أوردر برقم فاتورة.
  - تسليم أوردر.
  - إنهاء الخروجة.
  - تسجيل مشوار داخلي.
- إضافة SQL functions أساسية:
  - `delivery_search_customers`
  - `delivery_start_attendance`
  - `delivery_start_run`
  - `delivery_add_order`
  - `delivery_finish_run`
  - `delivery_calculate_payroll`
- إضافة RLS Pilot-friendly بدون recursion حتى لا يتعطل تسجيل الدخول.

### ترتيب الاختبار المقترح

1. نفّذ `20_stability_product_upgrade.sql`.
...
3. اعمل Redeploy.
4. ادخل بـ `DR.MOAZ / 9493`.
5. افتح `/delivery/rider`.
6. اضغط تسجيل حضور.
7. اضغط بدء خروجة.
8. ابحث عن عميل تجربة مثل `عميل` أو `C001`.
9. أضف رقم فاتورة.
10. اضغط تم التسليم.
11. اضغط إنهاء الخروجة.
12. راجع `/delivery` و `/delivery/orders`.

### ملاحظة أمان

سياسات RLS الحالية في ملف `20_stability_product_upgrade.sql` مناسبة لمرحلة Pilot لتشغيل التطبيق بسرعة. بعد ثبات الـ Flow، يفضّل تشديد السياسات بحيث يرى كل Rider بياناته فقط، ويملك Admin فقط صلاحيات الإدارة الكاملة.
